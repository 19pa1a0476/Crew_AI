                    ...
                    try:
                        ...
                        for doc in batch_docs:
                            try:
                                # doc.embed(embedder=self.embedder)
                                cleaned_content = self._clean_content(doc.content)
                                # Include content + filters (kb_name + user_id) in hash for uniqueness
                                import json
                                filters_str = json.dumps(filters, sort_keys=True)  # stable string representation
                                hash_input = cleaned_content + filters_str
                                content_hash = md5(hash_input.encode()).hexdigest()
                                _id = doc.id or content_hash
                                record = {
                                    "id": _id,
                                    "name": doc.name,
