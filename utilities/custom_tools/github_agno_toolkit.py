...
class GithubTools(Toolkit):
    def __init__(
        ...
        search_repositories: bool = False,
        ...
        create_general_pr_comment: bool = True,
        ...
        **kwargs,
        ...
        super().__init__(
            name="github",
            tools=[
                self.get_github_tree,
                self.get_file_content,
                self.create_general_pr_comment,
        ...
        if get_github_tree:
            self.register(self.get_github_tree)
        if delete_repository:
            self.register(self.delete_repository)
        if list_branches:
            self.register(self.list_branches)
        if create_general_pr_comment:
            self.register(self.create_general_pr_comment)
    ...
    def create_general_pr_comment(
        ...
        """
        Create a general conversation comment on a pull request.
        ...
        Returns:
            A JSON-formatted string with the comment info or an error message.
        """
        log_debug(f"Adding general comment to PR #{pr_number} in repository: {repo_name}")
        try:
    ...
    def get_github_tree(self, repo_url: str, branch: str = None) -> str:
        """
        Fetch the recursive file tree of a GitHub repository using the GitHub API client.
        Args:
            repo_url (str): Github repo url.
            branch (str): Branch name.
        Returns:
            str: A JSON-formatted string with file paths and types, or an error message.
        """
        log_debug(f"Fetching file tree for repository: {repo_url}, branch: {branch}")
        ...
        try:
            ...
            repo = self.g.get_repo(repo_name)
            if not branch : branch = repo.default_branch
            ref = repo.get_branch(branch=branch)
            sha = ref.commit.commit.tree.sha
            ...
            logger.error(f"Unexpected error in get_github_tree: {e}")
            return json.dumps({"error": str(e)})
    ...
    def create_pull_request(
        ...
        Args:
            ...
            head (str): Branch with your changes (e.g., 'feature-x' or 'user:feature-x').
            base (str): Branch to merge into (e.g., 'main').
            draft (bool, optional): Whether the PR is a draft.
            maintainer_can_modify (bool, optional): Whether maintainers can modify the PR.

        Returns:
            A JSON-formatted string containing the created pull request details or error details.
        """
        ...
        try:
            ...
            # 1. Validate head != base
            if head == base:
                return json.dumps(
                    {"error": f"Head branch '{head}' cannot be the same as base branch '{base}'"}
                )

            # 2. Handle fork syntax (owner:branch)
            head_owner, head_branch = None, None
            if ":" in head:
                head_owner, head_branch = head.split(":", 1)
            else:
                head_branch = head

            # 3. Validate base branch exists in target repo
            try:
                repo.get_branch(base)
            except Exception:
                return json.dumps({"error": f"Base branch '{base}' does not exist in {repo_name}"})

            # 4. Validate head branch
            if head_owner and head_owner != repo.owner.login:
                # Cross-repo PR (from fork)
                fork_repo = self.g.get_repo(f"{head_owner}/{repo.name}")
                try:
                    fork_repo.get_branch(head_branch)
                except Exception:
                    return json.dumps(
                        {"error": f"Head branch '{head_branch}' does not exist in fork '{head_owner}/{repo.name}'"}
                    )
            else:
                # Same repo PR
                try:
                    ...
                    return json.dumps(
                        {"error": f"Head branch '{head_branch}' does not exist in {repo_name}"}
                    )
